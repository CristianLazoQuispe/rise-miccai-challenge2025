# =========================
# Inference CASCADE 3D con TTA Robusto
# =========================
import os, re, gc
import numpy as np
...
def run_cascade_inference(
    ...
    ROI_SIZE_FINE = (dim//2, dim//2, dim//2)  # Mismo tamaño, no reducir
    SW_BATCH = 2
    ROI_MARGIN = 25
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(test_loader, desc="Processing cases")):
            try:
                ...
                subject_id = None#batch.get("ID", [None])[0] if "ID" in batch else None
                print("subject_id:",subject_id)
                print("os.path.basename(img_path):",os.path.basename(img_path))
                if subject_id is None:
                    # Extraer ID del nombre del archivo
                    basename = os.path.basename(img_path)
                    basename = basename.split("_ciso")[0]
                    if "testing_":
                        subject_id =  basename.split("testing_")[-1]
                    if "validation_":
                        subject_id =  basename.split("validation_")[-1]
                    "TESTING_"
                    """
...
python inference_cascade.py \
  ...
  --models_dir /data/cristian/projects/med_data/rise-miccai/task-2/3d_models/predictions/eff-dice_ce_spatial/fold_models \
  --output_dir /data/cristian/projects/med_data/rise-miccai/task-2/3d_models/predictions/eff-dice_ce_spatial/submissions_fixed_v2 \
  --model_name eff-b2 \
  --dim 192 \
  --use_tta 1 \
  --use_post 1 \
