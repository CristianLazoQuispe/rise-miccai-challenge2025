        ...
        for batch in pbar:        
            ...
            for p, g in zip(preds_np, gts_np):
                ...
                for cls, key in [(1, "L"), (2, "R")]:
                    ...
                    if not np.isnan(dsc):  fold_metrics[f"dice_{key}"].append(dsc*2)
                    else:
                        fold_metrics[f"dice_{key}"].append(0)
                    if full:
            ...
            for p, g in zip(preds_np, gts_np):
                ...
                for cls, key in [(1, "L"), (2, "R")]:
                    ...
                    if not np.isnan(dsc):  fold_metrics[f"dice_fine_{key}"].append(dsc*2)
                    else:
                        fold_metrics[f"dice_fine_{key}"].append(0)
                    if full:
...
def save_debug_images(x, img_roi, lbl_roi, pred_lab1, pred_lab2, y, i, dim,
                      exp_name, out_dir="images", figsize=(18, 3), dpi=200,prefix=""):
    # 1) Ensure output folder exists
    save_dir = os.path.join(out_dir, exp_name)
    os.makedirs(save_dir, exist_ok=True)

    # 2) Fixed-size figure (1 row × 6 cols)
    fig, axes = plt.subplots(1, 6, figsize=figsize, dpi=dpi, constrained_layout=True)

    # 3) Plot slices (use nearest for labels/preds to avoid smoothing)
    axes[0].imshow(_to_np(x)[i, 0, :, dim//2, :], cmap="gray", interpolation="nearest")
    axes[1].imshow(_to_np(img_roi)[0, 0, :, dim//4, :], cmap="gray", interpolation="nearest")
    axes[2].imshow(_to_np(lbl_roi)[0, 0, :, dim//4, :], interpolation="nearest")
    axes[3].imshow(_to_np(pred_lab2)[:, dim//4, :], interpolation="nearest")
    axes[4].imshow(_to_np(y)[0, 0, :, dim//2, :], interpolation="nearest")
    axes[5].imshow(_to_np(pred_lab1)[:, dim//2, :], interpolation="nearest")

    for ax in axes:
        ax.axis("off")

    # 4) Save BEFORE showing (and don’t call plt.show() in training loops)
    out_path = os.path.join(save_dir, f"{exp_name}_{prefix}_sine_wave_{i:04d}.png")
    fig.savefig(out_path, bbox_inches="tight")  # tight crops excess padding
    plt.close(fig)
    return out_path
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(df, groups=df["ID"])):
        ...
        if use_train:
            ...
            for epoch in range(num_epochs):
                    ...
                    with torch.cuda.amp.autocast(enabled=True):
                        ...
                        if True:
                            ...
                            for i, bb in enumerate(bboxes):
                                ...
                                """      
                                if cnt_train==1:     
                                    path = save_debug_images(x, img_roi, lbl_roi, pred_lab1, pred_lab2, y, i, dim, exp_name,"train")
                ...
                val_dice_fine_L = val_fold_metrics["val_dice_fine_L"]
                ...
                scheduler_fine.step(val_dice_fine_L)
                if val_dice_fine_L > best_dice_fine:
                    best_dice_fine=val_dice_fine_L
                    torch.save(fine_model.state_dict(), os.path.join(fold_dir, "best_fine_model.pth"))
                    print(f"Nuevo mejor modelo guardado - Avg Fine Dice: {best_dice_fine:.4f}",os.path.join(fold_dir, "best_fine_model.pth"))
